...
func TestNewClient_NormalRequestContent(t *testing.T) {
	...
	mockClient.MockRequest("/api/notes/create", func(request *http.Request) (*http.Response, error) {
		...
		var noteRequest map[string]interface{}
		...
		if noteRequest["text"] != "content" {
			t.Errorf("expected content = content; got = %s", noteRequest["text"])
		}

		if noteRequest["i"] != "thisistoken" {
			t.Errorf("expected api token = thisistoken; got = %s", noteRequest["i"])
		}
